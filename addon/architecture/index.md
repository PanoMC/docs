...
The UI hash matters mainly for **released** addons: a new version ships a new zip, the hash changes, and the cache-busting query pulls the fresh bundle. During development the mechanism is different — when Pano runs in a development environment it rebuilds your UI zip from disk on each request (the hash becomes a `dev-build` sentinel rather than a content hash), and [Development Mode](/addon/getting-started/) tells the theme to re-fetch that bundle every request instead of caching it. That combination — development environment *and* Development Mode — is what makes a `bun run dev` change show up on F5; neither alone is enough on a stock release instance. The [Frontend Development](/addon/frontend/) tutorial covers what you do inside `onLoad()`.
