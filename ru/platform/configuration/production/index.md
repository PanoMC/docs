...
- [Режим обслуживания](../../maintenance/) проверен заранее — вы знаете своё право обхода и свой URL входа
- `server.trusted-proxies` заполнен, **если Pano работает за обратным прокси**. Если оставить список пустым,
  Pano полностью игнорирует `X-Forwarded-For`, и все запросы выглядят приходящими от прокси — блокировка IP и
  ограничение частоты для входа в режиме обслуживания увидят один адрес вместо адресов посетителей
- Если через тот же обратный прокси подключаются **Minecraft-серверы**, убедитесь, что тайм-аут простоя
  WebSocket больше, чем heartbeat Pano (`heartbeat-interval` / `heartbeat-timeout`, по умолчанию **25s** /
  **75s**) — см.
