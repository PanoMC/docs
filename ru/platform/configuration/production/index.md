...
- Если через тот же обратный прокси подключаются **Minecraft-серверы**, убедитесь, что тайм-аут простоя
  ...
  **75s**) — см.
  ...
  Если вы меняете эти значения, держите их в пределах диапазонов, описанных там — пара вне диапазона
  не применяется: Pano записывает предупреждение в лог, возвращается к значениям по умолчанию 25 с /
  75 с и запускается нормально
