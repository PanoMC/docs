...
Arayüz hash'i esas olarak **yayınlanmış** eklentiler için önemlidir: yeni bir sürüm yeni bir zip gönderir, hash değişir ve önbellek-kırıcı sorgu taze paketi çeker. Geliştirme sırasında mekanizma farklıdır — Pano bir geliştirme ortamında çalıştığında arayüz zip'inizi her istekte diskten yeniden derler (hash, bir içerik hash'i yerine bir `dev-build` işaretine dönüşür) ve [Geliştirme Modu](/tr/addon/getting-started/), temaya o paketi önbelleklemek yerine her istekte yeniden getirmesini söyler. Bu kombinasyon — geliştirme ortamı *ve* Geliştirme Modu — bir `bun run dev` değişikliğinin F5'te görünmesini sağlayan şeydir; stok bir yayın örneğinde ikisinden biri tek başına yeterli değildir. [Arayüz Geliştirme](/tr/addon/frontend/) eğitimi, `onLoad()` içinde neler yaptığınızı kapsar.
