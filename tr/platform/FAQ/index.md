...
Evet. Kurulum sihirbazının ilk ekranında **Sunucu yönetimi** seçeneğini seçin ya da daha sonra **Panel → Ayarlar → Platform → Tercihler** bölümünden bu moda geçin. Bu durumda asıl ürün panel olur: **Yazılar**, **Talepler** ve **Görünüm** (temalar) kapatılır, Pano temayı hiç başlatmaz ve herkese açık tüm adresler `/panel` adresine yönlendirilir; panelin orada [kendine ait bir giriş sayfası](../server-management/#using-pano-without-a-website) vardır. Bu tercih `config.conf` dosyasında [`usage-mode`](../configuration/#genel-ayarlar) anahtarı olarak saklanır.
