...
- **Motor** (`@panomc/theme-core`) — *çalışan* her şeyi bu yapar: giriş, kayıt, eklentiler, sunucunuzdan veri yükleme ve siteyi derleme. Bunların hiçbirini siz yazmazsınız. Bir paket olarak gelir ve onu diğer paketler gibi güncellersiniz.
...
├─ core-meta.json         ← oto     temanızın kullandığı motor katmanı
├─ package.json           ← oto     bağımlılıklar (svelte motora sabitlenmiş)
├─ svelte.config.js       ← oto     motora tek satırlık çağrı
├─ vite.config.js         ← oto     motora tek satırlık çağrı
...
Bu dosyalar motor tarafından sizin **için** oluşturulur. Onları asla elle düzenlemeyin — bir sonraki `bun run sync` veya motor güncellemesi bunların üzerine yazar ve değişiklikleriniz kaybolur.

| Dosya veya klasör | Nedir |
|---|---|
| `src/routes/` | Motordan yeniden üretilen sayfa bağlantıları. |
| `src/lib/` | Motordan yeniden üretilen yardımcı kod ve köprüler. |
| `lang/` | Her dilin temel metni (metni bunun yerine `lang-overrides/` ile değiştirin). |
| `hooks.server.js`, `hooks.client.js` | Motoru çağıran küçük kurulum dosyaları. |
...
## "Motor güncellenir" sizin için ne demek

Motor bir paket olduğu için, onu güncellemek yalnızca iki komuttur:
...
"Sizin" listesindeki her şey — renkleriniz, metniniz, varlıklarınız — bir güncellemeden etkilenmez. Motorun yeni özelliklerini ve düzeltmelerini alırken temanız tam olarak yaptığınız gibi görünmeye devam eder.
:::
